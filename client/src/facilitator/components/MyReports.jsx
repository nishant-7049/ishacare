...
import { getSessionsForFacilitator } from "../../store/slices/dashboardSlice";

const MyReports = () => {
  const dispatch = useDispatch();
  const { dropouts, facilitatorSessions } = useSelector(
    (state) => state.dashboard
  );
  const [recoveryRate, setRecoveryRate] = useState();
  const [sessionDuration, setSessionDuration] = useState();
  useEffect(() => {
    dispatch(getSessionsForFacilitator());
    if (dropouts) {
  ...
  useEffect(() => {
    if (facilitatorSessions) {
      let min = 0;
      let sec = 0;
      let sessionsLength = 0;
      for (let ts of facilitatorSessions) {
