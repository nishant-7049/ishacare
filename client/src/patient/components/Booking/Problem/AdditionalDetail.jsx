...
const AdditionalDetails = ({activeStatus, setActiveElement, problem, setProblem, setCompletedSections}) => {
  const [isActive, setIsActive] = useState(false)
  const [isCompleted, setIsCompleted] = useState(false)
  const submitHandler = (e)=>{
    e.preventDefault()
    if(problem?.problem == "Multiple sclerosis" || problem?.problem == "Paralysis"){
    ...
    <div className='flex flex-col gap-4'>
      ...
      {isActive && (
        <form className='flex justify-center items-center flex-wrap gap-4 mx-auto w-full' onSubmit={submitHandler}>
          {inputForProblem.cause.includes(problem?.problem) && problem?.problem == "Paralysis"? <div className='flex flex-col sm:w-1/3 w-1/4 grow'>
            <label>Cause *</label>
            <select className='p-1 bg-white border-2 rounded-md' value={problem?.cause} required onChange={(e)=>{
              setProblem((prev)=>({
                ...prev,
              ...
              {paralysisCauses.map((cause, index)=>(
                <option value={cause} key={index}>{cause}</option>
              ))}
            </select> 
          </div>:
          ...
          {problem?.cause == "Other" && <div className='flex flex-col sm:w-full w-1/4 grow'>
            <label>Other cause *</label>
            <input maxLength={50} placeholder="Enter patient's cause if not available in cause." required type="text" className='p-1 bg-white border-2 rounded-md' value={problem?.otherCause} onChange={(e)=>{setProblem((prev)=>({
              ...prev,
              otherCause: e.target.value
            }))}} />
          </div>}
          {inputForProblem.inv.includes(problem?.problem) && <div className='flex flex-col sm:w-1/3 w-1/4 grow'>
            <label>Investigations done *</label>
            <select className='p-1 bg-white border-2 rounded-md' value={problem?.inv} required onChange={(e)=>{
              setProblem((prev)=>({
                ...prev,
                inv: e.target.value
              }))
              ...
              {investigations.map((investigation, index)=>(
                <option value={investigation} key={index}>{investigation}</option>
              ))}
            </select> 
          </div>}
